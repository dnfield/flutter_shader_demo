...
/*
    ...
    The effect itself is nothing new or exciting, just some moving 3D Voronoi
   layering. However, the fire palette might prove useful to some.
...
// This is my favorite fire palette. It's trimmed down for shader usage, and is
// based on an article I read at Hugo Elias's site years ago. I'm sure most old
// people, like me, have visited his site at one time or another:
//
...
vec3 firePalette(float i) {
  ...
  vec3 L =
      vec3(7.4, 5.6,
           4.4); // Red, green, blue wavelengths (in hundreds of nanometers).
  L = pow(L, vec3(5)) * (exp(1.43876719683e5 / (T * L)) - 1.);
  return 1. - exp(-5e8 / L); // Exposure level. Set to "50." For "70," change
                             // the "5" to a "7," etc.
}
...
// Hash function. This particular one probably doesn't disperse things quite as
// nicely as some of the others around, but it's compact, and seems to work.
//
vec3 hash33(vec3 p) {
...
float voronoi(vec3 p) {
  ...
  vec3 b, r, g = floor(p);
  p = fract(p); // "p -= g;" works on some GPUs, but not all, for some annoying
                // reason.

  // Maximum value: I think outliers could get as high as "3," the squared
  // diagonal length of the unit cube, with the mid point being "0.75." Is that
  // right? Either way, for this example, the maximum is set to one, which would
  // cover a good part of the range, whilst dispensing with the need to clamp
  // the final result.
  float d = 1.;

  // I've unrolled one of the loops. GPU architecture is a mystery to me, but
  // I'm aware they're not fond of nesting, branching, etc. My laptop GPU seems
  // to hate everything, including multiple loops. If it were a person, we
  // wouldn't hang out.
  for (int j = -1; j <= 1; j++) {
    for (int i = -1; i <= 1; i++) {
...
void main() {
  ...
  col = mix(col, col.zyx * .15 + c * .85,
            min(pow(dot(rd.xy, rd.xy) * 1.2, 1.5), 1.)); // Color dispersion.
