...
float hash(in float s) { return fract(sin(s * 43758.5453123)); }
...
vec3 flameheart(in vec2 cnt) {
  ...
  float TTime = iTime;
  cnts.y -=
      (0.5 + 0.1 * sin(TTime * 0.1) + 0.05 * sin(TTime * 0.23)) * abs(cnt.x);
  ...
  float ampl = 512.;
  8. * (0.5 -
        noise(vec2(ra.y * 8., tt), vec2(8., tt + 100.))); // sin(time)*0.03;
  ff = pow(1. - abs(ra.x - rad) +
               ampl * pow(fbm(vec3(ra.y * 32., ra.x * 8., tt * 2.),
                              vec3(32., 8., tt * 2. + 1000.)),
                          8.),
           .5);
  ...
  col = mix(blflame, flame,
            1. - pow((rads.x - rad) * (rads.x - rad) / (rad * rad), .5));
